...
export default function AiSummarizerClient() {
  ...
  const handleFiles = useCallback((newFiles: File[]) => { addFiles([newFiles[0]], true); setSummary(null); }, [addFiles]);
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      const summaryText = [
        `📄 **Document:** ${title}`, `✍️ **Author:** ${author}`,
        ...
        pageCount === 1 ? 'It appears to be a single-page document.' : pageCount <= 5 ? 'Given its short length, it is likely a memo or short report.' : pageCount <= 20 ? 'The moderate length suggests an article or detailed report.' : 'This is a substantial document — potentially a research paper or manual.',
        '', '> 💡 *For real AI summaries, connect to an LLM API like OpenAI or Google Gemini.*',
      ].filter(Boolean).join('\n');
      setProgress(100); setSummary(summaryText); toast.success('Summary generated!');
    } catch (err) { console.error(err); toast.error('Failed to analyze PDF.'); }
    finally { setProcessing(false); }
  }, [files, setProcessing, setProgress]);

  const handleReset = useCallback(() => { reset(); setSummary(null); }, [reset]);

  return (
    <ToolPageLayout title="AI Summarizer" description="Analyze and summarize your PDF metadata instantly.">
        ...
        <div className="p-6 rounded-xl bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-500/5 dark:to-purple-500/5 border border-indigo-200 dark:border-indigo-500/10 animate-slide-up">
          <div className="prose prose-sm dark:prose-invert max-w-none">
            {summary.split('\n').map((line, i) => {
              ...
              return (<p key={i} className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{parts.map((part, j) => part.startsWith('**') && part.endsWith('**') ? <strong key={j} className="font-semibold text-gray-900 dark:text-white">{part.slice(2, -2)}</strong> : <span key={j}>{part}</span>)}</p>);
            })}
          </div>
        </div>
      )}
      <ActionPanel onProcess={handleProcess} onDownload={() => {}} onReset={handleReset} processing={processing} progress={progress} hasResult={false} canProcess={files.length > 0 && !summary} processLabel="Analyze PDF" />
    </ToolPageLayout>
  );
}
